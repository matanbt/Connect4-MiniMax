...
def minimax(board, depth,maximizing_player=True, alpha = -math.inf, beta=math.inf):
    ...
    if maximizing_player:
        ...
        for col in range(BOARD_COLS):
            ...
            if status=="error_col_is_full": continue
            #terminal cases:
            elif status == "move_won": curr_score=1000*depth #the faster the better
            elif status == "full_table": curr_score=0
            #recursive case:
        ...
        for col in range(BOARD_COLS):
            ...
            if status == "error_col_is_full": continue
            # terminal cases:
            elif status == "move_won": curr_score = -1000*depth
            elif status == "full_table": curr_score = 0
            # recursive case:
            else:  # status=="moved_succ"
...
def draw_status(SCREEN, player, turns):
    ...
    draw_text(SCREEN, "Player " + str(player) + "'s Move", size=int(0.25*SQUARE_SIZE), coor=(int(0.8 * WIDTH), int(0.25*SQUARE_SIZE)), bold=False)
    draw_text(SCREEN, "Turns: " + str(turns), size=int(0.25*SQUARE_SIZE), coor=(int(0.8 * WIDTH), int(0.6*SQUARE_SIZE)), bold=False)
...
def main():
    ...
    while not gameover:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button==1:  # make move
                ...
                if res[0] == "full_table" or res[0] == "move_won":
                    ...
                    cou += 1
                    if last_move_drawn is not None: draw_move(SCREEN, last_move_drawn[1], last_move_drawn[0], player,False)
        ...
        if gameover:  # last words
            ...
            draw_text(SCREEN,"Click For Another Game", size=int(0.4*SQUARE_SIZE),coor=(WIDTH//2 , int(HEIGHT*0.6)),border=True)
